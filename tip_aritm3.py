...
# Prepoznaje i množenje kod kojeg * nismo naveli
...
class AritmParser(Parser):
    ...
    def član(self):
        f1 = self.faktor()
        sljedeće = self.granaj(Ar.KRAJ, Ar.PUTA, Ar.PLUS, Ar.ZATVORENA, Ar.OTVORENA)
        if sljedeće == Ar.PUTA:
            self.pročitaj(Ar.PUTA)
            f2 = self.član()
            return AST(stablo='umnožak', lijevo=f1, desno=f2)
        elif sljedeće == Ar.OTVORENA:
            f2 = self.izraz()
            return AST(stablo='umnožak', lijevo=f1, desno=f2)
        else:
            return f1
    ...
    def faktor(self):
        p1 = self.potencija()
        sljedeće = self.granaj(Ar.KRAJ, Ar.POT, Ar.PUTA, Ar.PLUS, Ar.ZATVORENA, Ar.OTVORENA)
        if sljedeće == Ar.POT:
...
def testiraj(izraz):
    ...
    python_izraz = python_izraz.replace(')(',')*(')
    for i in range(len(python_izraz)):
        if python_izraz[i].isdecimal() and i < len(python_izraz)-1 and python_izraz[i+1] == '(':
            python_izraz = python_izraz[:i+1] + '*' + python_izraz[i+1:]
...
if __name__ == '__main__':
    ...
    testiraj('(3+3)(4+1)')
    testiraj('(4+1)*5')
    #testiraj('(4+1)5') ne prepoznaje kad je broj nakon zagrade a * ispustena
